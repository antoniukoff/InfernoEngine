...
namespace Inferno {
	...
	void SpriteFont::init(const char* font, int size, char cs, char ce) {
		...
		for (int ri = 0; ri < bestRows; ri++) {
			...
			for (size_t ci = 0; ci < bestPartition[ri].size(); ci++) {
				...
				if (4 * glyphSurface->w < glyphSurface->pitch) {
					glPixelStorei(GL_UNPACK_ROW_LENGTH, glyphSurface->pitch / 4);
				}
				else {
					glPixelStorei(GL_UNPACK_ROW_LENGTH, 0);
				}

				glTexSubImage2D(GL_TEXTURE_2D, 0, lx, bestHeight - ly - 1 - glyphSurface->h,
	...
	void SpriteFont::draw(SpriteBatch& batch, const char* s, glm::vec2 position, glm::vec2 scaling,
		float depth, ColorRGB8 tint, Justification just /* = Justification::LEFT */) {
